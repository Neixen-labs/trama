...
def compile_geojson(source: Path, destination: Path) -> None:
    ...
    node_ids = sorted({node_id for edge_id in edge_ids for node_id in edges[edge_id][0]})
    node_index = {node_id: index for index, node_id in enumerate(node_ids)}

    tiles: dict[tuple[int, int, int], list[int]] = {}
    for edge_index, edge_id in enumerate(edge_ids):
        tiles.setdefault(_fit_tile(edges[edge_id][1]), []).append(edge_index)
    ...
    decoded = [
        (b"GEOM", *tile, _geometry_section(tile, [(index, edges[edge_ids[index]][1]) for index in tiles[tile]]))
        for tile in tile_keys
    ]
    decoded += [
        (b"GRPH", 0, 0, 0, _graph_section(node_ids, node_index, edge_ids, edges, geometry_refs)),
        (b"PROP", 0, 0, 0, _property_section()),
...
def _read_edges(document: dict) -> dict[int, tuple[tuple[int, int], list[tuple[float, float]]]]:
    """Map every LineString feature to a stable edge id, its endpoint node ids, and its metres."""
    edges: dict[int, tuple[tuple[int, int], list[tuple[float, float]]]] = {}
    for feature in document.get("features") or []:
        geometry = feature.get("geometry") or {}
        if geometry.get("type") != "LineString":
            raise ValueError("v0 compiler accepts LineString features only")
        if feature.get("properties"):
            raise ValueError("v0 compiler does not support properties yet")
...
def _graph_section(
    ...
    for edge_index, edge_id in enumerate(edge_ids):
        source_id, target_id = edges[edge_id][0]
    ...
    edge_records = b"".join(
        struct.pack(
            "<QIIIIII",
            edge_id,
            node_index[edges[edge_id][0][0]],
            node_index[edges[edge_id][0][1]],
            edge_index,
            edge_index,
            1,
            0,
...
def _property_section() -> bytes:
    """Empty PROP: no keys, no columns, three empty dictionaries."""
    header_size = 40
    columns_offset = header_size + 12
    header = struct.pack(
        "<10I", 0, 0, 0, 0, 0, header_size, header_size + 4, header_size + 8, columns_offset, columns_offset
    )
    return header + struct.pack("<3I", 0, 0, 0)
