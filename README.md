...
| `core/trama-swmm` (Rust) | The third domain: a SWMM drainage network — junctions, outfalls, storage, conduits, pumps, weirs — imports via `--importer swmm`, exports back to its own dialect with hydrology carried unread, and **simulates**: `trama-solver-swmm` runs EPA SWMM 5.2.4 — vendored and statically linked, since no `swmm-sys` existed to reuse — and streams node depth and link flow over SSE. Round trip verified by simulation. | the browser build (WASI) |
