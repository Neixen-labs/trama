...
| `core/trama-swmm` (Rust) | The third domain: a SWMM drainage network — junctions, outfalls, storage, conduits, pumps, weirs — imports via `--importer swmm` and exports back to its own dialect, hydrology carried unread. Each EPA importer redirects the other's file instead of mis-parsing it. | the engine — no `swmm-sys` exists on crates.io, so binding it is its own decision |
