...
| `core/trama-power` (Rust) | The fourth domain: a pandapower network — buses, lines, transformers — imports through `--importer power`, keeping every electrical parameter as a typed property and the rest of the network in one opaque record. Declares `voltage` (p.u.) and `loading` (%). Two buses drawn at one coordinate, which is how a substation's high and low sides arrive, are separated by ten metres so the transformer between them survives as an edge. | — |
