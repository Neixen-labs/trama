...
| `core/trama-epanet` (Rust) | `.inp` import and export, and a solver that runs the EPANET 2.3 toolkit and streams pressure, flow and **water age** — how long water sits in the network, the question utilities are regulated on — natively or in the browser through WASI. Round trip verified by simulation on Net1 and Net3. | chlorine decay and source tracing, the other two EN_QUALITY analyses |
| `core/trama-swmm` (Rust) | The third domain: a SWMM drainage network — junctions, outfalls, storage, conduits, pumps, weirs — imports via `--importer swmm`, exports back to its own dialect with hydrology carried unread, and **simulates**: EPA SWMM 5.2.4 — vendored and statically linked, since no `swmm-sys` existed to reuse — runs natively behind `trama-solver-swmm` over SSE, and in the browser through WASI: the playground compiles a SWMM `.inp` and solves it without a server. Round trip verified by simulation. | — |
