...
## 2026-08-16 — A fleet, and an oracle built rather than borrowed

**Decision:** `trama-routing` gains `fleet`, the capacitated vehicle routing problem: several vehicles from one depot, a demand at every stop, a capacity per vehicle. Clarke-Wright savings for the construction, 2-opt for the improvement, exposed as a second solver — `fleet-routing`, with its own `fleet.toml` and its own binary.

**Why a second solver rather than more parameters on the first:** a route is told where to go and in what order; a fleet is told what must be served and works the order out. Folding them together would have meant one manifest whose parameters are mutually exclusive, which is a JSON Schema admitting it describes two things. They share a Dijkstra — extracted as `explore`, with the early exit made optional, since a route wants one destination and a fleet wants every distance — and share nothing else.

**This is the first solver here that returns a good answer rather than the answer.** Every other one computes something with a right value: a load flow converges or it does not, a fault current is what it is. A VRP is NP-hard and Clarke-Wright is a heuristic, so "it works" is a claim that needs a number attached.

**So the oracle was built instead of borrowed.** There is no pandapower for this. `tests/fleet.rs` enumerates every way of splitting the stops between the vehicles and every order within each split — `vehicles^stops` labellings times the permutations — and computes the true optimum by exhaustion on an instance small enough to allow it. The heuristic is then measured against that. On the fixture it lands *on* the optimum; the test asserts a tenth and prints the real gap, so a regression that stays inside the bar is still visible to anyone reading the output.

**Two bugs the comparison found, both in the measurement rather than the algorithm**, which is the usual way round and worth recording: the test's own distance matrix was built with the fleet planner, so every distance came back doubled — a round trip is not a leg — and the planner's matrix took its distances from the Dijkstra queue, whose values are scaled to thousandths for ordering. Rebuilt from the edge costs themselves, the two agree exactly. The second was a real defect: it made the fleet's idea of a distance disagree with the single-route planner's over the identical path.

**The `vehicle` channel declares no range**, unlike the four `trama-roads` already had. Those are readings between nothing and everything; this one is an identity, and how many vehicles a fleet has is the caller's question. A file that guessed a maximum would have the host reject the fifth van's deltas as invalid.

**Consequence:** 4 tests on the planner and one on the manifest. Time windows are the obvious next constraint and are deliberately absent: they change the problem from "which stops, in what order" to "which stops, in what order, arriving when", and every part of the construction above would need to know about feasibility in time. That is a different piece of work, not a parameter.

## 2026-08-17 — The machines that hold a voltage, and the ones that stop

**Decision:** `trama-power` gains PV buses. A pandapower `gen` becomes a bus whose voltage magnitude is imposed and whose reactive power is free, within the limits the machine declares; when a machine is asked for more than it has, it is pinned at its limit and its bus becomes a PQ bus for the rest of the study. `shunt` is read too. The solver reports which machines gave up, in the order they did.

**Why this one next:** it is what the crate's own source called the natural next addition, and it is the difference between a distribution solver and one that can also read a transmission network. Embedded generation behaves as negative load and needed nothing; a station on automatic voltage regulation does not, and every network above the feeder has one.

**The limit is the point, not the PV bus.** A PV bus alone is a smaller change than it looks: the Jacobian was already blocked for it, and it only means one fewer unknown and one fewer mismatch. What earns the work is what happens at the limit. A generator asked for more reactive power than it can deliver stops controlling voltage, and the reactive power it stops supplying is picked up by whichever machines are still regulating — which is how a voltage collapse actually propagates. On IEEE 14-bus at 120% load, two machines exceed their limit outright and pinning them pushes a third past its own. A single pass finds the first two and reports a network held up by a machine that gave up.

**Control is never handed back.** A pinned machine released to regulate again would be released into the conditions that saturated it, and the two states would alternate for as long as anyone let them. This is also what PYPOWER and pandapower do, which matters because agreement with the reference is the property under test — but it would be the right choice with no reference at all.

**Two tables were being ignored in silence, which is the finding worth recording.** Neither `mv_oberrhein` nor CIGRE MV carries a `gen` or a `shunt`, so nothing in the test suite noticed that the importer read neither. A capacitor bank compiled into a container simply disappeared, and the load flow returned a voltage the real network only reaches because the bank is there — a plausible number, with nothing anywhere saying a component had been dropped. The lesson is not about these two tables: an importer that skips what it does not recognise fails silently by construction, and only a fixture that carries the unrecognised thing will ever say so.

**The transformer rating was measured against the wrong quantity, and one network could not tell.** `loading` for a transformer was computed as apparent power over the nameplate; pandapower's default is current against each winding's rated current, `i·vn·√3/sn`. The two agree exactly where the bus sits at the winding's nominal voltage, which is why 2 transformers on `mv_oberrhein` matched to 1e-6 points and hid it. IEEE 14-bus, whose buses sit well away from nominal, disagreed by 0.017 points — small, real, and in the direction of understating a loaded transformer. A winding burns on current.

**A fault on a network with generators is refused rather than answered.** A synchronous machine feeds a short circuit from behind its subtransient reactance, with a correction factor of its own in IEC 60909 §3.6, and none of that is modelled here. Leaving the machines out would still produce a number: lower than the truth, entirely plausible, and the basis for choosing a breaker that fails closed on the day it is needed. So the study stops and says what it does not model, the same way a missing `s_sc_max_mva` already did. The load flow on the same file is unaffected — the refusal is about the question, not the network.

**Consequence:** on IEEE 14-bus, 14 bus voltages within 1e-8 p.u. and every machine's reactive output within 1e-6 Mvar of `pandapower.runpp`, both at nominal load and at 120% with `enforce_q_lims=True`, where three of the four machines saturate in two passes. 32 tests in the crate. `Model.shunts` is gone: an admittance to earth now lives on the bus that carries it, so the load flow's capacitor bank and the fault study's source impedance are one thing to the matrix instead of two paths that could disagree.

## 2026-08-18 — The generator as a source, and the network that had to be written

**Decision:** `trama-power` models a synchronous machine's contribution to a short circuit. Under `Study::Fault` a `gen` is no longer refused but becomes an admittance to earth — `Z_G = r_dss + j·x″d·vn²/sn`, corrected by IEC 60909 §3.6's `K_G` — exactly as an external grid already was. A machine that declares no `xdss_pu` is still refused, and so is a power station unit, whose §3.7 correction is computed over the machine and its step-up transformer as a pair.

**Why this one next:** the previous entry closed by refusing this study, which was the honest thing to do and not a resting place. On any network with a generator on it, the machine is the second largest source of fault current after the infeed — on the fixture here, faulting a generator's own bus gives a current more than a third higher than the same network without it. Refusing was right; leaving it refused would have been a gap with a note on it.

**It cost almost nothing structurally, and that is the point of the previous entry.** Unifying shunts onto `Bus.y_shunt` last time — so the load flow's capacitor bank and the fault study's source impedance were one thing to the matrix — is what made this a new formula and no new machinery. The generator lands in the same field, `thevenin` was already reading it, and nothing else changed.

**Both ratios in `K_G` were exercised on purpose.** The factor is `(vn_bus/(vn_gen·(1+pg%))) · c_max/(1 + x″d·sin φ)`, and its first two terms are 1 on a machine rated at its own bus's voltage and running at rating — which is the machine most fixtures would contain. So the network here carries one of those *and* one rated at 21 kV on a 20 kV bus running 5% above rating. A solver that dropped the ratios would match the first and miss the second, which is the failure a single-machine fixture would have certified as correct.

**The fixture had to be written, and that is the finding worth recording — again.** No published benchmark carries `xdss_pu`. `mv_oberrhein` has no generators; IEEE 14-bus has four and declares a reactance for none of them; CIGRE MV has none. This is the same shape as `s_sc_max_mva` a study earlier: the data a fault needs is data a load flow never reads, so a file that was only ever used for load flow does not carry it. The network in `generators.json` is therefore written rather than downloaded — four buses, an infeed, a transformer, two lines and two machines, small enough that every current in it was checked by hand against the algebra before the test was allowed to assert anything. The oracle is still `pandapower.shortcircuit.calc_sc`; only the network is ours.

**Consequence:** 4 bus fault currents within 1e-9 relative of `calc_sc`, and a second test asserting the machines actually raise the current rather than merely being read — a solver that parsed the table and did nothing with it would pass a comparison whose reference was generated the same broken way. 34 tests in the crate. `mv_oberrhein`, CIGRE MV and IEEE 14-bus are unchanged.

## 2026-08-19 — The turn nobody may take, and the state that had to change

**Decision:** `trama-roads` reads OSM `type=restriction` relations and writes them as a `PROP` column, `roads:no_turn`, holding for each edge the ids of the edges it may not be followed by. `trama-routing` and its fleet honour that column when a caller names it in `restriction_property`, the same way `speed_property` already names the column that turns a shortest path into a fastest one.

**Where it lives was the decision, and the spec made most of it.** A turn restriction is a fact about a *pair* of edges, which is not a shape any single section obviously holds. `XTRA` was the tempting answer and is explicitly the wrong one: SPEC §7.1 forbids topology in it and requires that removing every `XTRA` record leave a file that "traverses identically" — a restriction changes exactly that. A new section would have been the formally correct home and a spec PR before any code. The column is what was chosen: it needs no format change, it has a precedent in the same crate, and it keeps the router domain-free, because "this edge may not be followed by those" is a statement about a graph and not about a road.

**Dijkstra over nodes stops being correct, and that is the substance of the change.** Settling nodes assumes the cheapest path to a node is part of the cheapest path through it. A turn restriction is precisely the case where that fails: arriving at a junction along one street and along another are different situations, because they permit different exits. So the search now settles *arcs* — an edge together with the direction it was crossed in, which the CSR adjacency already enumerates — and retraces along arcs too. Retracing by node would have been free to pick a different arc into each junction than the search used, which under a restriction is not another path of equal cost but a path through a turn the search refused.

**A test found that the ids were being written to the wrong name, and it would have failed silently.** An edge's id is the digest of `edge:{declared}`, not of the declared string itself. The importer's own tests could not catch it: they compared the column against the same function that wrote it, which is a tautology whichever string it hashes. The end-to-end test through a compiled container is what disagreed. The failure mode this avoided is the quiet kind — `forbidden_turns` skips ids that name no edge on purpose, because a bounding box cuts through relations, so every restriction in every file would have resolved to nothing and no route would ever have changed. The rule now lives in `trama_format::edge_id`, namespace included, and is imported rather than reproduced.

**`only_*` is expanded at import rather than carried as a mode.** `only_straight_on` forbids every exit but one, which is the same statement as a list of prohibitions. Expanding it once, where the junction's other exits are already known, means the reader answers one question — may I go from here to there — and never has to know which spelling produced the answer.

**Consequence:** 26 tests in `trama-roads` and 24 in `trama-routing`, including the one that pins why arcs are the state: a network where the cheap way into a junction cannot leave it, so the route must arrive by the dearer road. A `from` way that runs *through* its `via` node — which OSM asks to be split and does not always get — has the restriction applied from both sides: forbidding a turn that was allowed costs a longer route, allowing one that was not sends a driver the wrong way up a junction.

**Restrictions whose `via` is a way are skipped**, and that is a real hole rather than a rounding: a no-U-turn across a dual carriageway spans two junctions, and no property of one edge can express it. Those need the path-shaped state this design does not have, which is the same reason they are the minority OSM itself treats as a separate case.

## 2026-08-19 — The restriction reaches the page, and the example that had none

**Decision:** `solve_route` in `trama-wasm` takes a `restriction_property`, the playground names `roads:no_turn` when it calls it, and `fixtures/teruel.trama` is regenerated from a query that asks for the restriction relations as well as the ways.

**Why it needed doing at all:** the importer had been writing the column since that morning, and nothing read it. A file compiled in the browser carried every turn Teruel forbids and the route drove through them, which is the same failure as not having implemented it, arrived at from the other side. `KICKOFF.md` puts the criterion on the page rather than on the crate for exactly this reason.

**The example is where the work actually was.** The playground's road example is a *compiled* container, checked in, and its Overpass query asked for `way(area.city)` and nothing else — so no relation was ever in the file to compile. Adding `relation(area.city)["type"="restriction"]` to the query is the whole change, and it costs nothing in the container: the ways are the same ways, the graph is the same graph, and 69 edges gain a column. What did move is the graph itself, by two nodes and two edges, because OSM is a live database and the previous snapshot was a week old. That is worth stating plainly in `fixtures/README.md` rather than being discovered as a regression.

**A real extract found a bug two synthetic tests had agreed was fine.** Of Teruel's 151 restrictions, 65 are `no_u_turn` — a relation whose `from` and `to` name the *same* way. The importer dropped exactly that case: it filtered the arriving pieces out of the forbidden ones, which is right for `only_*` (where turning back is not one of the exits being ranked) and wrong for `no_*` (where turning back is the entire point). Both spellings had a test and both passed; neither test used the same way twice. The fix is one condition, and the lesson is that the tests were written from the shapes the code already handled.

**The isochrone still spreads through forbidden turns**, and the doc comment on `solve_route` says so. `trama-trace` runs its own search, which settles nodes; honouring a restriction needs the arc-settling one this crate's router moved to. Making that change is a piece of work with its own answer to the same question, not a parameter to thread through.

**Consequence:** 28 tests in `trama-roads`. Verified in the browser rather than argued: over the shipped container, routing between the same two nodes crosses 20 edges with the column ignored and 21 with it named, and the playground draws the route with the scrub running. Of 67 random pairs across the city one differs, which is what 69 restricted edges out of 3,651 should do.

## 2026-08-20 — The isochrone learns the turn the route already knew

**Decision:** `trama-trace` takes a `restriction_property` naming the same `roads:no_turn` column `trama-routing` reads, and its search settles arcs rather than nodes so that it can honour it. `solve_reach` in `trama-wasm` passes the column through and the playground names it, so the isochrone and the route drawn over one container no longer disagree about which movements exist.

**This closes the gap the previous entry left open in writing.** That entry ended by saying the isochrone still spread through forbidden turns and that fixing it was a piece of work with its own answer to the same question. This is that answer, and it turned out to be the same one: a node stops being a state under a restriction, so the state has to be the arc.

**Duplicating the search was the decision, and it was made deliberately over sharing it.** `trama-routing::explore` already settles arcs, and reusing it would have meant one implementation instead of two. It would also have meant growing that function by four parameters — multi-seed, budget, reversed direction, blocked edges — none of which its own caller uses, because a route wants one path and stops as soon as it settles while a spread wants every edge's cost and must run the queue dry. The shared version is more total code than the forty lines it saves. A `ponytail:` comment on `forbidden_turns` names the duplication and the condition for undoing it: a third reader, not a second.

**What `Backward` and `Both` do was the other decision, and it is not symmetric.** A restriction is stated the way a driver meets it — come in along this edge, that exit is shut. Running the network against the arrows is the same statement read from the far end, so the relation is inverted rather than dropped: having crossed the exit, the entry is what may not be taken. Ignoring direction is not a movement at all. `Both` answers "what is connected to this", which has no turn in it to forbid, and applying one there would quietly answer a different question — which is also why `Isolation`, whose default is `Both`, is unaffected: a one-way street still has water under it, and a no-left-turn does not stop a valve from feeding a pipe.

**The test that matters is the one that fails for the right reason.** Asserting that a restricted spread reaches fewer edges proves nothing: a search that settled nodes *and* checked restrictions would pass it, because it too would refuse the turn — it would simply also lose the legitimate arrival behind it. So the test lifts the budget and asserts the arrival cost of the edge past the junction: three hops, by the dearer road, rather than absent. Simulating node-settling against it was tried, and it reports `None` where the arc search reports `Some(3.0)` — a street the spread can plainly reach, called unreachable. Over the shipped city the property is stated one-directionally: forbidding a movement can only make an edge dearer or unreachable, never cheaper, so the restricted isochrone is a subset of the unrestricted one at every budget, which breaks in one direction if the search loses edges and in the other if it leaks through a junction.

**Consequence:** 21 tests in `trama-trace` and 8 over the fixture. The effect is much larger than the router's, and for a reason worth recording: a route is one pair of points and 67 random pairs across Teruel produced one difference, while an isochrone touches the whole network at once — at a ten-minute budget, 47 of 75 sampled starting points spread differently. Verified in the browser over the shipped container: from one point, 2,544 streets within ten minutes with the column ignored and 2,536 with it named, and the playground draws the spread with the scrub running.

## 2026-08-20 — The stop that is not open yet

**Decision:** `trama-routing`'s fleet takes time windows. Each stop may declare `[opens, closes]` and a service time; arriving early is a wait the clock shows, arriving late is a refusal. Every pass that could produce an undrivable round — the Clarke-Wright construction, the 2-opt improvement, and the plan's own entry check — asks the same function whether a route can be driven.

**Early and late are not symmetric, and that asymmetry is the whole model.** A vehicle can stand still; it cannot arrive sooner. So a window is a hard ceiling and a soft floor, which is the standard VRPTW and not a simplification: treating early arrival as infeasible too would discard rounds any driver could drive and leave many real instances with no solution at all. Soft windows — late allowed at a penalty — were considered and refused for v0: they change the objective from time to time-plus-λ·lateness, and with it the meaning of `total_cost` and of the comparison against the true optimum this crate already makes.

**One definition, three callers.** `schedule` lays the clock along a route and returns the service times or nothing. The construction asks before merging two rounds, the improvement asks before keeping a reversal, and the assembly asks in order to put the waiting into the clock the map draws. Writing the rule once is not tidiness here: the clock that decides a round is drivable and the clock the scrub animates have to be the same number, or the playground shows a van arriving somewhere the planner believed it did not.

**Capacity is a sum and time is not.** A merge's load can be checked on the two totals; whether the merged round can be driven depends on the order its stops end up in, so the only way to know is to walk it. That makes the construction O(n³) where the plain problem is O(n²) — fine for the tens of stops a van does in a day, and marked with a `ponytail:` comment naming the fix if anyone ever plans hundreds: carry each route's latest feasible start alongside its load and the check is O(1) like the capacity one beside it.

**2-opt survives, filtered, and it is worth saying what that costs.** Reversing a run reorders every arrival behind it, which is exactly the move a window is most likely to refuse — so with tight windows most candidate reversals are rejected and the plan lands close to what the construction alone produces. Or-opt, which moves a short run without reversing it, is the move that survives windows and is the obvious next step; it was not written here because the filter is a few lines against its twenty-five and the quality it leaves on the table is not yet measured. That is a deferral with a name, not an oversight.

**Four guards, four tests, verified by breaking each one.** The habit from the isochrone paid again: with the first draft of the suite, removing the improvement pass's feasibility check changed nothing and no test noticed, because on a ladder no reversal both shortens a round and breaks a window. The instance was rebuilt with one leg made dear in a single direction — as a one-way street would — so that the strictly cheapest order is the one the windows forbid, and the check is now exercised as a unit test over a hand-written matrix. Removing any of the four guards now turns exactly one test red.

**Infeasibility is reported where it is provable.** A stop the depot cannot reach before its own close is refused up front, naming the stop and the earliest arrival, because the direct run is the earliest any vehicle could manage and every later pass only adds time. Without that, the planner would quietly return rounds violating the constraint it was handed, and "every stop served exactly once" would still pass.

**Consequence:** 12 tests in `trama-routing`'s fleet, including the optimum by exhaustion re-run over feasible orders only — the oracle now enumerates permutations and filters them through a second, independent implementation of the window rule written in the test file, so the check is on the answer and not on the code agreeing with itself. A fleet with no windows takes the same path through the code it did before and the existing optimality comparison is unchanged.

**Not done, deliberately:** a window on the depot — "the van must be back by six" — which would bound the round rather than the stops, and soft windows. Neither is hard on top of this; both are scope nobody has asked for yet.

## 2026-08-20 — The roads the query never asked for

**Decision:** `fixtures/teruel.overpassql` asks for `*_link` ways as well, `speed_metres_per_second` gains the road classes it was missing — `motorway`, `trunk`, and the five link classes — and `fixtures/teruel.trama` is regenerated from both.

**This was found while costing a different piece of work, and that is why it is written down first.** The agreed next task was `via`-way turn restrictions. Before designing anything, the volume was measured: Teruel has 1 of 153, Zaragoza 10 of 1,001, Madrid 24 of 2,964 — between 0.7% and 1.0%, and Madrid full of dual carriageways does not move the number. The same count then asked where the *other* dropped restrictions were going, and 68 of Teruel's 153 turned out to name a way the extract simply did not contain. Of the 80 missing ways, 73 are slip roads.

**The query was the bug, and the missing restrictions were the smaller half of it.** Slip roads are how a city joins its fast roads. Leaving them out left parts of the network reachable only in principle: the largest connected component was 95.2% of the streets, and is now 99.0% — from 25 components down to 17. Of 52 sampled node pairs, four had no route at all before and do now. A map cannot show this, which is the same lesson the twelve-hundred-fragment extract taught and the reason `components` exists.

**The speed fallback was a second, independent error the first one hid.** `speed_metres_per_second` had cases for `living_street`, `residential`, `unclassified` and `primary`/`secondary`/`tertiary`, and everything else fell to 30 km/h — including `trunk`, of which the extract has 265 ways with only 29 declaring a `maxspeed`. So 236 stretches of dual carriageway were costed as residential streets. `motorway` was in the same hole and got away with it, declaring a limit on 62 of its 64 ways. Every route and isochrone that used a trunk road was wrong in the same direction, and nothing in the output said so: a plausible number is the hardest kind of error to see, which is precisely the failure mode `docs/DECISIONS.md` recorded for transformer loading and for edge ids.

**A slip road is priced between the roads it joins**, not as either. Giving a link its parent's speed makes every interchange look like a shortcut; leaving it on the residential default makes the only legal way onto a motorway look like a back street. Both distort the same junction in opposite directions, so `motorway_link` is 60, `trunk_link` 50 and the rest 40 — heuristics, stated as such, and beaten by any declared `maxspeed` like every other class.

**Consequence:** the published example goes from 2,772 nodes and 3,651 edges to 2,870 and 3,855, 466.0 km to 488.7 km, and 241 kB to 256 kB. Edges carrying a `roads:no_turn` column go from 69 to 135. Of 52 node pairs routable in both files, 31 are now faster and 17 unchanged; total driving time across them falls 42.8%, almost all of it the trunk roads no longer being residential streets. Verified in the browser over the rebuilt container: a route the length of the city, 32.83 km, drawn with the scrub running. 30 tests in `trama-roads`, two of them new and both of which fail against the old fallback.

**The `via`-way restriction is still refused, now with a number on it.** Under 1% of restrictions in three cities of very different sizes, against a design that needs path-shaped state and a format decision. It stays the next piece of work rather than a closed one, but the measurement is recorded here so the next person to reach for it knows what it buys.

## 2026-08-21 — The turn that spans two junctions

**Decision:** a turn restriction becomes a *run* of edges rather than a pair. `roads:no_turn` gains a `>` separator so a cell can hold runs of any length, `trama-format` gains a shared `Turns` automaton over those runs, and both searches settle an arc together with how far along the runs it stands. The `via`-way restriction — the no-U-turn across a dual carriageway — is read and honoured.

**No spec change was needed, and finding that out was the first job.** The plan of record said this needed a `docs/SPEC.md` PR before any code, because the restriction spans two junctions. It does not: §5 makes `PROP` typed columns carrying no domain semantics, and §7.1 forbids putting in `XTRA` anything another section can express. A run of ids in a string column is exactly as expressible as a pair of ids was, and the precedent was set by the pair. What spans two junctions is the *search state*, not the storage — those were conflated in the note that deferred this, and the note was wrong about which one.

**A pair was always a run of length two**, so the column extends rather than doubles: a lone id still means what it always meant, already-published files read unchanged, and there is one concept where there would have been two. The reader distinguishes them by the separator and nothing else.

**The walk carries its progress, which is what makes any length work.** A search cannot ask "were the last four edges this run?" without carrying the last four, and the number of ways to arrive somewhere along four edges is bounded by nothing useful. So the state is `(arc, progress)`, progress being a position in an Aho-Corasick automaton over the runs. The fail links are load-bearing: having walked `a, b` a search is partway along `a, b, c` *and* partway along `b, d`, and dropping either loses a restriction. A file declaring no runs builds a one-state automaton that every step returns to, so the common case pays nothing and needs no branch to get that.

**The duplication was undone, against the criterion written for it.** #193 left the arc search copied in `trama-routing` and `trama-trace` with a `ponytail:` comment saying to extract on a *third* reader. There is no third reader. What changed is the size of what had to be copied: twenty lines of parsing became an automaton, and a miscopied automaton is two crates silently disagreeing about which movements a file permits. The criterion was the number of readers; the one that mattered was the size of the shared thing.

**Two blind spots, both found by breaking the code rather than reading it.** Neither would have been caught by the tests as first written:

- Collapsing the progress to zero after every step — checking the automaton but not carrying it — broke nothing in `trama-trace`. Every restriction in its tests bit on the first step out of the seed, where the progress is computed separately. The fix was a test whose run bites on the *second* step, and where the same three edges are forbidden from one approach and allowed from another.
- Settling arcs alone, ignoring the progress in the visited set, broke nothing either. That is the node-settling bug of #193 one level in: reaching an edge cheaply partway through a run and dearly clear of it are different situations, and keeping only the cheap one reports a street unreachable that the search can plainly reach. Both crates now have the test, and both fail without the distinction.

**The dense state array had to go, and only measuring showed it.** Teruel's automaton has 329 states over 3,855 edges, so `arcs × states` allocated and cleared about 10 MB per search: 30 isochrones went from 14 ms to 110 ms, an eight-fold cost for a feature that changes one street in the city. Held per arc instead — nearly every arc is reached at one progress only — the same 30 run in 22 ms. The algorithm was never the problem; the allocation was.

**Consequence:** 34 tests in `trama-roads`, 22 in `trama-trace`, 20 in `trama-routing`, 7 for the automaton itself, each of the last built by breaking a part of it. The published Teruel container carries 136 edges with the column against 135, and its longest run is now three edges rather than two — that one edge is the whole visible effect, which is what 1 restriction in 153 buys and was known before the work started. Verified in the browser over the rebuilt container: routes and isochrones both run, four sampled routes differ on two of them, and the automaton costs no measurable time in WASM.

**Still refused:** nothing, in the restrictions. The `via`-way was the last known gap, and the count that justified doing it last is in the entry above.

## 2026-08-21 — The transformer with three ends, and the tap nobody was reading

**Decision:** a three-winding transformer becomes three edges meeting at a node the source file did not have. `trama-power` writes that star point as a bus of its own, under the negative of the transformer's index, and splits the nameplate's three pair impedances into the three branch impedances the star needs. `trafo3w` joins `bus`, `line` and `trafo` as a table the importer expresses.

**pandapower already answers the design question, and copying its answer is what makes the numbers comparable.** It solves a three-winding transformer by building three two-winding ones against an auxiliary bus that exists in its calculation and not in `net.bus`. The alternative — a delta of three edges between the three real buses, with no new node — is electrically equivalent seen from outside and was refused for two reasons: the currents in a delta's branches are not the currents in the windings, so `loading` would stop meaning the thing that burns, and a tap at the star point has nowhere to go. The star point costs a node that is not in the source, and buys a per-winding loading that can be compared against `res_trafo3w` one number at a time.

**The impedances are not a third each, and one of them is usually negative.** A nameplate gives the short-circuit voltage of each *pair* of windings with the third open, each measured on the smaller rating of its own pair. Referring all three onto the high winding's rating and running the delta-star transform is what turns three pairwise measurements into three branch impedances. The middle winding of a real machine sits close enough to the other two that its share comes out below zero — in the fixture here, twice. That is the model and not an error to guard against, so the sign is carried through `vk_percent` rather than a magnitude being rebuilt from it.

**IEC 60909's §3.7 correction goes on the pairs, before the split.** The standard corrects the impedance of a pair of windings, which is what the nameplate declares; applying it to the three branches that come out of the split corrects a quantity the standard never names. Measured by removing it: every fault current moves 1.5%, in the direction that makes a breaker look adequate.

**Two things about tap changers turned up that had nothing to do with three windings, and both were wrong for two-winding transformers too.** The first fixture draft left `tap_changer_type` empty while setting `tap_pos` to three — which is what `create_transformer3w_from_parameters` does unless you say otherwise — and the comparison came apart. pandapower moves a ratio only where the file *names* a changer; this solver was reading the position alone, which puts up to 9% of voltage on a bus that does not have it. The second is in pandapower's own source, in one line: under `mode == "sc"` it returns the untapped voltages outright, because IEC 60909 computes the fault through the nominal ratio and the correction factor is what stands in for the network not being at that voltage. Putting the tap back counts the same departure twice, and it was worth 13% of fault current.

**Neither of those was visible, and that is the part worth keeping.** `cigre-mv.json` and `generators.json` both declare an empty `tap_changer_type`, and `oberrhein.json` declares `Ratio` on both of its transformers with a fault study nobody runs on it. So the existing fixtures agreed with pandapower for reasons unrelated to the code being right, and every one of them stayed green through both bugs. Telling an empty column from an absent one needs the table *schema*, which is already in `XTRA` because the remainder keeps the columns of every expressed table — a file written before pandapower 3.0 has no such column at all, and there the position is still all there is to read.

**Consequence:** 6 tests in `trama-power`'s new `trafo3w.rs`, against a network written for it — `example_multivoltage` is the only published pandapower network with a `trafo3w`, and it also carries an `impedance`, two `xward`s and four open bus-bus switches, none of which this solver models. Every bus voltage including both star points agrees to 1e-8 p.u., every winding loading to 1e-6 points, every fault current to 1e-9 relative. Each of the three guards was measured by breaking it: 1.5%, 8.5% of voltage, 13% of fault current. `Rating::Winding` now takes an end as `Option`, because the branch reaching a star point has one real winding and one end inside the machine. **Not done, deliberately:** a tap changer at the star point, refused with a message rather than approximated — pandapower has the six lines it takes, and what is missing is a fixture that would catch getting them wrong.

## 2026-08-21 — The machine and its transformer are one machine

**Decision:** `trama-power` models a power station unit — a generator bolted to its step-up transformer, with nothing connected between them — under IEC 60909 §3.7, correcting the pair with a single factor instead of giving the machine `K_G` and the transformer `K_T`. The refusal that stood in `generator()` since the short circuit was written is gone.

**Two formulas, and which one applies turns on one column.** Where the unit transformer regulates on load, `K_S` squares the voltage ratios and *subtracts* the reactances — a regulating transformer holds the machine's terminal voltage, so what matters is how far the machine's reactance and the transformer's differ rather than the machine's alone. Where it can only be tapped off load, `K_SO` uses the ratios once and carries `(1 − p_t)`, how far those fixed taps reach. Treating them as one formula with a flag would be wrong in both directions.

**The network is not the same seen from inside the unit.** A fault on the machine's own terminals has the unit transformer *behind* it, so the pair correction the rest of the network is entitled to would be correcting for an impedance that is not in the path. That is why `Unit` survives into the model rather than being consumed while building it: `fault_currents` sweeps Thévenin once for the network and once more per unit, undoing `K_S` on that one machine and its transformer and taking exactly one number from each extra sweep. Measured by not doing it: 6.0%.

**A generator terminal sits at the machine's voltage, not at its bus's.** The last thing to make the comparison land was not a formula but a base: `c·Un/√3` at a unit's machine bus is referred to the machine's rated voltage, and pandapower says so in one line at the end of `_calc_ikss` with the comment "Correct voltage of generator bus inside power station". Worth exactly `V_G/Un` — 4.8% on this fixture, where 10.5 kV machines sit on 10 kV buses. Every correction factor had already been checked against `ppc["bus"][:, K_G]` and `[:, K_SG]` and agreed to ten digits, which is what made it obvious the error was in the application and not in the physics — the three buses outside the units were exact and the two machine terminals were both wrong by the same 1.05.

**What this cost, against what it buys.** Declaring a unit rather than a machine-plus-transformer moves the fault current by 0.25% to 1.9%, measured before writing anything. That is small — but the alternative was not a 2% error, it was no answer at all, because the solver refused any file declaring `power_station_trafo`. The cost is one Thévenin sweep per unit, and units are per station rather than per bus: a plant has a handful of sets.

**Consequence:** 3 tests in `trama-power`'s new `station.rs`, against a network written for it — no published pandapower network declares `power_station_trafo`. Two units, one with an on-load changer and one without, plus an ordinary machine behind an ordinary transformer as a control that must not move. Every bus within 1e-9 relative of `calc_sc`. Each guard measured by breaking it: 4.8% for the terminal voltage, 6.0% for the sweep inside the unit, 1.6% for letting the unit transformer keep its own `K_T` on top of `K_S`. `generator()` is now `machine` (the raw admittance), `alone` (§3.6) and `pair` (§3.7), which is the split the standard already makes. `Bus` and `Branch` derive `Clone`, because a per-unit sweep is a copy of the network with two entries changed.

## 2026-08-21 — The plan that existed and was refused, and the measurement that corrects an earlier note

**Decision:** the fleet planner gains a consolidation pass. When the savings construction leaves more rounds than the fleet has vehicles, the smallest round is taken apart and its stops are inserted into the others — at any position, not only at an end — cheapest feasible placement first, all or nothing. And a refusal now names the constraint that actually bites.

**This was found by measuring, and the measurement was the whole task.** #194 deferred Or-opt with a stated reason: 2-opt reverses a run, a reversal reorders every arrival behind it, so tight windows refuse most candidate reversals and "Or-opt is the move that survives windows and is the obvious next step". The plan of record was to measure that before writing it. `tests/gap.rs` does, by splitting the gap against the true optimum into *ordering* — what any intra-route move could win, computed exactly by reordering each planned round by exhaustion — and *assignment*, which no intra-route move touches.

**The prediction was backwards.** The ordering ceiling *falls* as windows tighten: 1.63% with no windows, 1.67% under shared morning/afternoon slots, and 0.00% under windows wrapped tightly around one order. A tight window leaves so few feasible orders that whichever one the planner lands on is already the best of them — the constraint does the heuristic's work. Or-opt is worth about a quarter of the gap on realistic instances and nothing at all on tight ones, which is a real number where there was a plausible story.

**What the same run found instead was worse than a missing improvement.** Under windows wrapped around a single order, the planner refused **46 of 60 instances that had a feasible plan** — zero of them genuinely infeasible. Clarke-Wright joins the end of one route to the start of another and nothing else, so a stop that would sit comfortably between two others is refused because neither end will take it, and the construction halts with more rounds than vans. Insertion in the middle is exactly the move that was missing, which is the same shape as Or-opt without being the same objective: it buys feasibility rather than distance.

**The message was wrong about which constraint failed, and that is its own defect.** "These stops need 3 vehicles at this capacity and the fleet has 2" was reported for eight stops of demand 1 against a capacity of 8 — capacity had nothing to do with it. A refusal now distinguishes a bound from a failure: total demand over capacity is a floor no arrangement can beat and is stated as such, while anything short of that floor says the planner could not find a plan, because it has not proved that none exists and the measurement says one usually does.

**A guard against consolidating for its own sake.** The pass runs only when there are too many rounds. Fewer vans is not the objective — the total is — and a stop moved into another round only ever adds to it. The no-windows row of the measurement is unchanged to the digit after the change, which is what says the guard holds.

**One correction to the measurement itself, recorded because it nearly became the finding.** The first run reported a 117% gap. `total_cost` is the clock at the end of a round and therefore includes the waiting a window imposes; the enumerated optimum sums legs. Comparing them charges the heuristic for standing still, which no reordering can avoid. Distance against distance gives 2-7%. A measurement can be wrong in the same way an implementation can, and a number that surprising is a reason to check the instrument first.

**Consequence:** `missed` goes from 46/60, 31/60 and 1/60 to **0/60** on all three window generators. The mean gap under wrapped windows rises from 5.81% to 7.48% — not a regression: the refused instances were the hard ones and now count towards the mean instead of being skipped. 4 unit tests in `fleet.rs` and 10 in `tests/fleet.rs`, two of them rewritten: the refusal test asserted only that the message contained the word "vehicles", which passed under either wording and under the bug. `tests/gap.rs` is `#[ignore]`d and carries its own numbers in its header, so the next person to reach for Or-opt starts from a measurement rather than from a prediction. **Or-opt is not done and now has a number on it rather than a story: about a quarter of the gap on realistic instances, none on tight ones.**
