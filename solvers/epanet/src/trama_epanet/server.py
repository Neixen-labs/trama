...
This is the second implementation of the same protocol, and it is deliberately a sibling of
the one in `solvers/example-diffusion` rather than a shared package. Two is not yet a pattern:
a third solver is when this framing belongs in a `trama-solver-runtime` of its own.
"""
