...
Known gaps, so they are not rediscovered as bugs: no WebGPU — deferred by decision, because MapLibre 6 hands a custom layer a WebGL2 context and only that one — no polygons, also deferred by decision to v1, no CSV point input, nothing published to npm or crates.io. `docs/DECISIONS.md` says why for each.
